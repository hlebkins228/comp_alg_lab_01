...
def calculate_ans_hermit(data: list[PointValues], current_x: float, current_nodes_count: int) -> float | None:
    derivatives_count = 3
...
def hermit_reverse_interpolation(data: list[PointValues]) -> float | None:
    reverse_data = list()
    for item in data:
